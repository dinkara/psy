...
class AuthController extends ApiController {
     ...
     * @return void
     */
    public function __construct(IUserRepo $userRepo, IProfileRepo $profileRepo, IRoleRepo $roleRepo, ISocialNetworkRepo $socialNetworkRepo, IDoctorRepo $doctorRepo, IPatientRepo $patientRepo) {
    ...
    public function register(StoreUserRequest $request) {
        ...
        try{
            ...
            if($request->file("avatar")){
                $profileData["avatar"] = $request->file("avatar")->store(config("storage.profiles.avatar"));
            }
            $this->profileRepo->create($profileData);

            if( $request->type === UserTypes::DOCTOR){
                $this->doctorRepo->create(['user_id' => $profileData["user_id"]]);
            }else{
                $this->patientRepo->create(['user_id' => $profileData["user_id"]]);
            }
